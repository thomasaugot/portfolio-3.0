...
export function HomeDifference() {
  ...
  return (
    <section id="difference" className="py-[clamp(80px,12vh,160px)] border-t border-border">
      <Shell>
        <div data-anim="section-head" className="grid grid-cols-[1fr_2fr] gap-gutter mb-14 items-end max-[720px]:grid-cols-1 max-[720px]:gap-4">
          <span className="text-caption font-mono text-text-subtle">{t("sections.difference_meta")}</span>
        ...
        <div className="grid grid-cols-3 gap-0 border border-border max-[900px]:grid-cols-1">
          {cards.map((card, idx) => (
            <div
              key={card.n}
              className={[
                "card-hover p-9 border-r border-b border-border flex flex-col min-h-75 max-[900px]:border-r-0",
