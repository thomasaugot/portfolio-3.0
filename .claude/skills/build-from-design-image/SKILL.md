...
> **THIS PROJECT — read before anything else.**
> There is no image-generation tool in this environment. Do not claim to have generated
> an image and do not describe one as if it exists.
>
> The workflow here is:
> 1. Write the art direction with `imagegen-frontend-web` (or `-mobile`) and hand Thomas
>    the prompts. He renders them in Claude Design.
> 2. He pastes the resulting image back into the chat.
> 3. THEN this skill applies: read the image closely and build the interface to match it.
>
> Step 3 without step 2 is guessing, which is exactly what produced four rejected designs.
> If no image has been provided, say so and stop rather than inventing a direction.
>
> Codex-specific instructions below apply to this environment unchanged in spirit: prefer
> large, readable, section-specific references over one compressed board.
