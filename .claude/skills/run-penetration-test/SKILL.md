...
description: Entry point and shared reference for all Strix security testing: installing the CLI, LLM/API-key setup, every CLI flag, scan modes, exit codes, reading results, and the managed-cloud alternative. Read this FIRST whenever `strix --version` fails, or when the target does not fit one of the specific workflows. For a running web app use test-web-app-security; for a REST/GraphQL/gRPC API use test-api-security; for source code use find-security-vulnerabilities-in-code; for an OWASP-mapped report use test-owasp-top-10; when the asset mix is unclear use plan-security-review.
license: Apache-2.0
metadata:
  author: usestrix
...
Summarize findings by severity (critical/high/medium/low/info) and include the PoC evidence. To remediate and verify fixes (via either path), use the **fix-security-vulnerabilities** skill. To wire scanning into CI/CD, use the **add-security-scanning-to-ci** skill.
