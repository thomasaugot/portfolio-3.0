---
name: test-web-app-security
description: Black-box pentest of a RUNNING web application - a live URL, staging environment, or local dev server - with Strix's autonomous agents, which exploit and prove each finding. Covers the authorization/scope checklist, credentialed testing behind login, and out-of-scope path handling. Use ONLY when the target is a deployed, reachable web UI. For an API endpoint use test-api-security; for a repo or local source tree use find-security-vulnerabilities-in-code; for setup and CLI flags see run-penetration-test.
license: Apache-2.0
metadata:
  author: usestrix
...
Install, LLM setup, all CLI flags, and the managed-cloud alternative are covered in the **run-penetration-test** skill — read it if the target is not a running web app, or if `strix --version` fails. This skill is the web-app-specific workflow.
...
Hand findings to the **fix-security-vulnerabilities** skill: patch the root cause, then re-run Strix against the same target to prove the exploit no longer works. Re-testing is the only reliable confirmation a fix landed.

To keep the app tested on every change rather than once, wire Strix into CI with **add-security-scanning-to-ci**.
