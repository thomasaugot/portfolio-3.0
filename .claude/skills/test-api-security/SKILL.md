...
description: Security-test a REST, GraphQL, or gRPC API against the OWASP API Security Top 10 (2023), where the bugs are authorization-shaped rather than injection-shaped. Covers schema-driven endpoint enumeration (OpenAPI/Swagger/Postman/proto) and the two-tenant credential setup that is the only way to PROVE BOLA/IDOR. Use ONLY when the target is an API or its schema. For a rendered web UI use test-web-app-security; for setup and CLI flags see run-penetration-test.
license: Apache-2.0
metadata:
  author: usestrix
...
Remediate with **fix-security-vulnerabilities** (fix the authorization check, not the single endpoint), then re-run against the same target to prove the exploit is dead. Wire it into pull-request CI with **add-security-scanning-to-ci** so new endpoints get tested as they ship.
