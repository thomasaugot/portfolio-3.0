...
description: Build an animation from scratch, making the decisions in the order that determines whether it feels right — should it animate at all, what purpose, which tool, which properties, which curve and duration, how it interrupts, how it exits. Writes the implementation. Use when asked to animate something, add motion, make a component feel alive, or build a transition. For critiquing existing motion use review-animations; for auditing a whole codebase use audit-animations.
---

# Building Animations

A construction skill. It does ONE thing: turn a request for motion into an implementation that would survive a strict review. It does not audit a codebase (that's `audit-animations`), critique a diff (that's `review-animations`), hunt for places that could animate (that's `find-animation-opportunities`), or build for React Native (that's `animate-expo`).
...
dropdown - **in this project you build it in `components/ui/` by hand**: third-party component
libraries are banned by rule 07-UI_PRIMITIVES, and `pick-ui-library` is deliberately NOT
installed here. The underlying warning still stands though: a hand-rolled `<div>` dropdown with
no focus management is a defect. Rule 07 specifies the full accessibility contract each
primitive owes (roles, keyboard handling, portal, focus trap) - meet it.
