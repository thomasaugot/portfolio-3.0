---
name: test-owasp-top-10
description: Produce a security report MAPPED TO THE OWASP TOP 10 categories (2025 edition for web, API Security Top 10 2023 for APIs) - for compliance, a customer security questionnaire, or an audit that must name categories. Includes an honest per-category coverage table stating what an agent can and cannot prove. Use ONLY when the OUTPUT must be OWASP-structured; for an ordinary pentest without that reporting requirement use test-web-app-security or test-api-security.
license: Apache-2.0
metadata:
  author: usestrix
...
Install, LLM setup, and the managed-cloud alternative: **run-penetration-test**.
...
For APIs, run the same exercise against the **OWASP API Security Top 10 (2023)** — API1 BOLA, API3 Broken Object Property Level Authorization (2019's excessive data exposure + mass assignment merged), API5 broken function-level authorization — using the **test-api-security** skill.
...
Remediate with **fix-security-vulnerabilities** and re-run to prove each exploit is closed. For ongoing coverage as the app changes, gate pull requests using **add-security-scanning-to-ci**.
