...
**Web mapping:** a critically damped spring (no overshoot) is the default; reserve bounce for
momentum-driven, physical interactions. **In this project that is GSAP, never Motion/Framer**
(rule 04) — GSAP has no `bounce` parameter, so express damping through the ease:

```js
import { gsap } from "@/lib/gsap"

// Critically damped default (no overshoot) — Apple's "damping: 1.0"
gsap.to(el, { y: 0, duration: 0.4, ease: "power3.out" })

// Momentum interaction — a little overshoot, only because a flick preceded it
gsap.to(el, { y: target, duration: 0.5, ease: "back.out(1.4)" })

// True spring physics when the interaction needs it (CustomEase/Physics2D
// are Club plugins; otherwise approximate with elastic)
gsap.to(el, { y: target, duration: 0.6, ease: "elastic.out(1, 0.75)" })
```
...
Example: element at `y=50`, target `y=150` (100px to go), finger moving 50px/s → initial spring velocity = `50 / 100 = 0.5`. GSAP has no velocity option: convert the release velocity into a duration and ease instead — a faster flick means a shorter duration, and `inertia` (Club plugin) handles it properly if available.
...
- **`prefers-reduced-motion: reduce`** — replace slides/springs/add-parallax with short opacity **cross-fades or static transitions**. Drop elastic/overshoot. Keep opacity/color changes that aid comprehension.
...
## THIS PROJECT: stack translation (read before applying anything above)

The principles above are tool-agnostic and they hold. The **tooling** references do not.
Where this section conflicts with anything above, this section wins. Where it conflicts with
`.claude/rules/`, the rules win.

| Upstream says | Here |
| --- | --- |
| Motion / `motion.dev` / `motion/react` | **GSAP**, imported only from `@/lib/gsap` (rule 04) |
| `useReducedMotion()` from Motion | `useMotionPreference()` from `@/contexts/MotionPreferenceContext`, or `window.matchMedia("(prefers-reduced-motion: reduce)")` inside `utils/animations/*` |
| `<motion.div>` / JSX-level animation | Pure functions in `utils/animations/`, targeted via `data-anim` attributes. **Never** `useRef` for a GSAP target (rule 05) |
| Inline `style` / `animate={{}}` props | BANNED. Tailwind classes only; GSAP writes inline styles itself and hand-written ones fight it (rule 03) |
| `pick-ui-library` for a toast/drawer/dropdown | BANNED. Primitives are hand-built in `components/ui/` (rule 07) |
| CSS custom easing tokens | Already defined in `styles/theme.css`: `--ease-out`, `--ease-in`, `--ease-inout`. Extend, do not fork (rule 03) |

### GSAP easing equivalents

Upstream's curves map onto GSAP's named eases. Use the GSAP name, not a hand-rolled bezier:

| Intent | Upstream curve | GSAP |
| --- | --- | --- |
| Entering / exiting (default) | `cubic-bezier(0.23, 1, 0.32, 1)` | `power3.out` or `expo.out` |
| Moving / morphing on screen | `cubic-bezier(0.77, 0, 0.175, 1)` | `power2.inOut` |
| Leaving the screen entirely | - | `power2.in` (exits only) |
| Constant motion | `linear` | `"none"` |

**"Never `ease-in` on UI" still applies**: `power2.in` / `power3.in` are for elements *leaving*,
never for an entrance. An entrance on an in-ease starts slow at the exact moment the user looks.

### Duration units

Upstream durations are in **milliseconds**; GSAP's `duration` is in **seconds**. A 200ms
dropdown is `duration: 0.2`. The under-300ms rule means `duration: 0.3` max for UI motion.

**Exception, already taken in this project:** the PageLoader and page transitions are
deliberately longer (a 1.8s minimum loader, ~0.6s curtain). Those are first-impression and
navigation moments, not repeated UI, so they sit in the "marketing / explanatory" tier.

### Mandatory cleanup

Every `init*` in `utils/animations/` MUST return its own cleanup function, and ScrollTriggers
must be killed there (rule 05). Upstream never mentions this because Motion unmounts with the
component; GSAP does not.
